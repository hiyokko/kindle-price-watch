...
test('store repair preserves authoritative mixed-edition child-list volumes over product title volumes', () => {
  const store = {
    books: [
      {
        id: 'shamo-1',
      ...
      },
      {
        id: 'shamo-4',
        asin: 'B00QAEZLAY',
        title: '極厚版『軍鶏』 巻之四 （１０～１２巻相当） (イブニングコミックス)',
        seriesName: '軍鶏',
        seriesKey: 'series:asin:B074CG522D',
        seriesExpectedCount: 22,
        importMode: 'kindle_series',
  ...
  assert.equal(store.books.find((book) => book.asin === 'B00SICO3PA').title, '軍鶏（２２） (イブニングコミックス)');
  assert.equal(store.books.find((book) => book.asin === 'B00WFOSQYG').volume, 22);
  assert.equal(store.books.find((book) => book.asin === 'B00WFOSQYG').title, '軍鶏（３４） (イブニングコミックス)');
