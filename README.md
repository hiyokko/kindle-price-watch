...
GitHub Actionsでは、`CHECK_INTERVAL_HOURS` より古い本を `CHECK_BATCH_SIZE` 件ずつ自動チェックします。既定のWorkflowでは毎日03:00 JSTに最大500冊、並列8で確認します。`CHECK_CONCURRENCY` を設定すると、1回の実行内で複数冊を並列に確認します。
